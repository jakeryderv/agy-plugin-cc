...
How a Claude Code session's context reaches an agy conversation: the single
SessionStart hook that records each session's transcript location, the lookup
that picks the right session for the current project, the extraction of
conversational turns from the transcript JSONL, and the handoff prompt that
transfers context without transferring initiative.

Two properties matter more than the mechanics. The hook must never be able to
break a user's session startup, and a transfer must never hand another
project's conversation to agy.
